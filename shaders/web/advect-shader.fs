...
float getHorizontalVelocity(ivec2 ij){
    ...
    if(right.x >= int(textureSizeOther.x)){
        right.x = int(textureSizeOther.x) - 1;
    }
    if(botRight.x >= int(textureSizeOther.x)){
...
float getVerticalVelocity(ivec2 ij){
    ...
    ivec2 current = ij;
    if(top.y >= int(textureSizeOther.y)){
    ...
    if(topLeft.y >= int(textureSizeOther.y)){
...
void main()
{
    ...
    if(mode == 0){          //horizontal advection
        ...
        y = clamp(y, 0.0, textureSize.y-0.1);
        ...
        }else if(isBoundary(ij) == 2 && ij.x >= int(textureSize.x)-1){
            FragColor = texelFetch(textureVx, ivec2(ij.x - 1, ij.y), 0).r;
        }
        //FragColor = res;
        //FragColor = velocityX;
        ...
        x = clamp(x, 0.0, textureSize.x-0.1);
        ...
        }else if(isBoundary(ij) == 2 && ij.y >= int(textureSize.y)-1){
            FragColor = texelFetch(textureVy, ivec2(ij.x, ij.y - 1), 0).r;
        }
        //FragColor = res;
        //FragColor = velocityY;
        ...
        vec2 uv = TexCoord;
        ivec2 ij = ivec2(uv * textureSize);
        float velocityX = texelFetch(textureVx, ij, 0).r + 10.0;
        FragColor = velocityX;
    }else if(mode == 4){
